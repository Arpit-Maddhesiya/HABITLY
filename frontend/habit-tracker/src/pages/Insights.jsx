...
export default function Insights() {
  ...
  const thisWeekLogs = useMemo(
    () => logs.filter((l) => thisWeekKeys.has(new Date(l.completedDate).toISOString().slice(0, 10))),
    [logs, thisWeekKeys]
  );
  ...
  const dailyData = thisWeek.map((d) => {
    const count = thisWeekLogs.filter((l) => new Date(l.completedDate).toISOString().slice(0, 10) === d.key).length;
    return { label: d.label, count };
  });

  const compareData = thisWeek.map((d, idx) => {
    const thisCount = thisWeekLogs.filter(
      (l) => new Date(l.completedDate).toISOString().slice(0, 10) === d.key
    ).length;
    const lastCount = lastWeekLogs.filter(
      (l) => new Date(l.completedDate).toISOString().slice(0, 10) === lastWeek[idx].key
    ).length;
