...
export const chatCompletion = async ({ system, user, temperature = 0.7 }) => {
  ...
  if (!c) {
    return {
      ok: false,
      content:
        "AI features are disabled - set GEMINI_API_KEY in the backend .env to enable real AI responses. Meanwhile here is a placeholder response.",
    };
  }
  try {
...
export const SYSTEM_PROMPTS = {
  ...
  recovery:
    "You are a compassionate habit recovery coach. The user broke a streak. Write a 3-day recovery plan tailored to this specific habits. Be warm but actionable. Use this structure: short empathetic opening (1-2 sentences), then Day 1 / Day 2 / Day 3 sections with one concrete action each, then a closing line of encouragement. 150-220 words total",
  chat: "You are a helpful habit analysis assistant. Answer the user's question using ONLY the provided habit data as conext. Be specific - cite actual habit names, days, percentages. Keep replies under 120 words. If the data is unsufficient, say so briefly.",
  morning:
    "You are a warm, motivating friend. Write a single short morning message (30-60 words) using the user's actual habit names and current streaks. Mention 1-2 specific habits. Energetic habits. Energetic but not cheesy. No emojis overload - max 1.",
};
