    ...
    if (!suggestions.length) {
      suggestions = [
        ...
        },
        {
          ...
          icon: "🙏",
          reason: "Tiny anchor habits that fits any schedule.",
        },
      ];
    }
    await AIInsight.create({
...
export const recoveryPlan = async (req, res) => {
  try {
    ...
    const userMsg = `Habit: ${habit.name} (${habit.category}).\nDescription: ${habit.description || "none"}.\nCurrent streak:${current} days, Longest ever: ${longest} days. The user just broke a strea. Write a warn, actionable 3-day recovery plan,`;
...
export const morningMotivation = async (req, res) => {
  try {
    ...
    const userMsg = `Today's habits and streaks:\n${ctx}\n\nDone today: ${done}/${total}. Write a short, motivating morning message that encourages the user to complete today's habits.`;
